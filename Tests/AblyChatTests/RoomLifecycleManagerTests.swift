...
final class RoomLifecycleManagerTests: XCTestCase {
    ...
    func test_attach_whenContributorFailsToAttachAndEntersSuspended_transitionsToSuspended() async throws {
        ...
        for error in await [suspendedStatusChange.error, manager.error, roomAttachError] {
            try assertIsChatError(error, withCode: .messagesAttachmentFailed, cause: contributorAttachError)
        }
    }
    ...
    // @spec CHA-RL1h6
    func test_attach_whenChannelDetachTriggered_ifADetachFailsItIsRetriedUntilSuccess() async throws {
        // Given: A room with the following contributors, in the following order:
        //
        // 0. a channel:
    ...
    func test_release_whenDetachFails_ifContributorIsFailed_doesNotRetry() async {
        ...
        // We use "did it call clock.sleep(…)?" as a good-enough proxy for the question "did it wait for the right amount of time at the right moment?"
        let clockSleepArguments = await clock.sleepCallArguments
        XCTAssertEqual(clockSleepArguments, [1_000_000_000])
